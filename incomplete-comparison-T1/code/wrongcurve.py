...
AtA=np.zeros((DIM,DIM)); Atb=np.zeros(DIM); done=0
for N in [30000,45000,60000,75000,85000,95000]:
    while done<N:
    ...
    w=int((np.clip(np.round(xi),-eta1,eta1)!=x).sum())
    print("N=%d: wrong=%d"%(N,w),flush=True)
